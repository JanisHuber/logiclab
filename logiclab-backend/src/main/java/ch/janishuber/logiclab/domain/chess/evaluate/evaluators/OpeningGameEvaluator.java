...
public class OpeningGameEvaluator implements BoardEvaluator {
    ...
    public int evaluateBoard(ChessBoard chessBoard, FigureColor botColor) {
        ...
        overallValue += getBishopPositionValue(chessBoard, botColor) / 2;
        return overallValue;
    }
    ...
    private static int getBishopPositionValue(ChessBoard chessBoard, FigureColor botColor) {
        int value = 0;
        for (Field field : chessBoard.getFields()) {
            if (field.getFigure() != null && field.getFigure().getClassName().equals("Bishop")) {
                int tempValue = (botColor == field.getFigure().figureColor) ? PieceTables.getBishopTableValue(field) : -PieceTables.getBishopTableValue(field);
                value += tempValue;
            }
        }
        return value;
